...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'AGIL-XHAVIER', 'ubuntu', '3.0' ]

	kurr.on('qr', () => {
	console.log(color('[','white'), color('!','red'), color(']','white'), color('Scan QR Nya Bang Berlaku 20 Detik !!!!'))
})
	...
	kurr.on('open', () => {
		success('2', 'Tersambung Jan Lupa Subs Yt Agil OFC')
	})
...
kurr.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6281394567319@s.whatsapp.net"
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6281394567319@s.whatsapp.net","6288971045303@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6281394567319@s.whatsapp.net"
